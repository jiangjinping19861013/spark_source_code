...
//  worker注册所有的master,一个是active一个是standby状态
...
//  向master注册worker请求消息，告诉master一个新的worker诞生
//        尝试向所有的master注册，获取master的url
...
//     worker 收到master 发送的执行创建执行器消息处理
//      实际上资源的分配都是在master上实现的，不过是通知worker去执行而已
...
//      判断master是否是存活的
      if (masterUrl != activeMasterUrl) {
        ...
        try {
          ...
          coresUsed += cores_
          memoryUsed += memory_
//          向Master发送ExecutorStateChanged的消息，告诉master excutor创建OK了
