...
//        master将业务的请求发给AppClient,driver发给woker进行具体处理
        driver.send(RegisteredApplication(app.id, self))
        schedule()
      }
    }

//      master 收到worker的ExecutorStateChanged信息
...
//        将worker信息持久化到master内存中
   ...
   */
//  在workers上分配executors
  private def startExecutorsOnWorkers(): Unit = {
    // Right now this is a very simple FIFO scheduler. We keep trying to fit in the first app
    // in the queue, then the second app, etc.
    for (app <- waitingApps if app.coresLeft > 0) {
//      可用的worker的标准是状态是alive的，其上并没有该Application的Worker，且Worker的内存资源是
//      充足的，优先选择core多的
...
//  master启动生成Executor信息，master掌控整个worker的资源信息如core数量和memory数量
  private def launchExecutor(worker: WorkerInfo, exec: ExecutorDesc): Unit = {
    logInfo("Launching executor " + exec.fullId + " on worker " + worker.id)
    worker.addExecutor(exec)
//    发送launchExecutor消息给Driver
