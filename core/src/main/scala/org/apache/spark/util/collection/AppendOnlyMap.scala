...
class AppendOnlyMap[K, V](initialCapacity: Int = 64)
  ...
  def changeValue(key: K, updateFunc: (Boolean, V) => V): V = {
    ...
    var i = 1
    while (true) {
      // 这里的data是一个数组，用来同时存储key和value
      // 2*pose 存储的是key 2*pos+1存储的是value
      // 从data中获取该位置的原来的key
      val curKey = data(2 * pos)
      if (k.eq(curKey) || k.equals(curKey)) {
        // 如果key已经存在则调用updateFunc更新value
        ...
        // 如果key不存在就将key和value的值放进数组中
        val newValue = updateFunc(false, null.asInstanceOf[V])
        data(2 * pos) = k
        data(2 * pos + 1) = newValue.asInstanceOf[AnyRef]
//        根据K的hashCode再哈希与上掩码 得到 pos，2 * pos 为 k 应该所在的位置，2 * pos + 1 为 k 对应的 v 所在的位置，
        // 获取k应该所在位置的原来的key：
//        若原来的key和当前的 k 相等，则通过update函数将两个v进行聚合并更新该位置的value
//        若原来的key存在但不和当前的k 相等，则说明hash冲突了，更新pos继续遍历
//        若原来的key不存在，则将当前k作为该位置的key，并通过update函数初始化该k对应的聚合结果，
        // 接着会通过incrementSize()方法进行扩容：
        incrementSize()
        return newValue
