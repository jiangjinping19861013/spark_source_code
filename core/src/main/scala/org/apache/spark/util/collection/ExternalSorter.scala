...
//  需要聚合的情况，遍历records拿到record的KV，通过map的changeValue方法并根据update函数来对相同K的V进行聚合，
//  这里的map是PartitionedAppendOnlyMap类型，只能添加数据不能删除数据，底层实现是一个数组，数组中存KV键值对的方式是[K1,V1,K2,V2...]，每一次操作后都会判断是否要spill到磁盘。
//
//  不需要聚合的情况，直接将record放入buffer，然后判断是否要溢写到磁盘
  def insertAll(records: Iterator[Product2[K, V]]): Unit = {
    ...
    if (shouldCombine) {
      ...
      // 如果需要进行map端的combine操作，使用PartitionedAppendOnlyMap作为缓存
      ...
      while (records.hasNext) {
        addElementsRead()
        kv = records.next()
        // 这里的map和下面else中的buffer都是用来缓存的数据结构
