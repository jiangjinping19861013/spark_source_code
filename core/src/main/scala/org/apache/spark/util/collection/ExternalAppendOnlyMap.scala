...
//    2. 这里非常重要的一点是这个next，这个next最终会在哪里停止呢？
...
//    2. 将读取map中数据和读取每个spillfile中的数据的iterator组合在一起
    private val inputStreams = (Seq(sortedMap) ++ spilledMaps).map(it => it.buffered)
//    3. 将数据全部读取出来，放到mergeHeap中队列中
...
//      1.由于PriorityQueue的特点，直接提取的一个key最小的buffer
