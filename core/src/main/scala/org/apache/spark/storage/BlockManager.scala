 ...
 * Note that #initialize() must be called before the BlockManager is usable.
  shuffle 客户端的shuffleClient
  BlockManager:对已所有的Executor上的BlockManager同一管理
  磁盘块管理器DiskBlockManager
  内存存储MemoryStore
  磁盘存储DiskStore
  Tachyon存储Tachyon
  非广播Block清理器metadaClearner和广播Block清理器broadcastCleaner
  压缩算法的实现CompressionCode
 */
...
private[spark] class BlockManager(
   ...
   */
//     BlockTransferService的初始化和ShuffleClient的初始化
//    blockManagerId和ShuffleServicId的创建
...
//    向BlockManagerMaster注册blockManagerId
   ...
   * memory on slave.
   */
//    向BlockManagerMasterActor报告Block的状态并且重新注册BlockManager
...
//          block能持久化到内存且磁盘上没有该block，则将该block内容持久化到磁盘
