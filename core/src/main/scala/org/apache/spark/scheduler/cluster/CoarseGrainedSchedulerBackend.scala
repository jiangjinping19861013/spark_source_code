  ...
  extends ExecutorAllocationClient with SchedulerBackend with Logging
{
  ...
  /*
  DriverEndpoint继承自ThreadSafeRpcEndpoint，所以DriverEndpoint是一个消息循环体，
  那么他到底负责与谁进行通信呢？我们继续追踪源码，实例化完成后回到CoarseGrainedSchedulerBackend中的start方法：
  向RpcEnv进行注册，注册的名字是ENDPOINT_NAME，即"CoarseGrainedScheduler"，
  注册的时候会调用DriverEndpoint的onStart方法(Rpc内部的机制决定的
*/
