...
class DAGScheduler(
   ...
   * the provided firstJobId.
    * stage是根据算子来划分的，找父stage
   */
  private def getParentStages(rdd: RDD[_], firstJobId: Int): List[Stage] = {
//    1.实例化一个空的集合用来存储ResultStage的父Stage
    val parents = new HashSet[Stage]
//   2. 实例化一个空的集合用来存储已经遍历过的RDD
    val visited = new HashSet[RDD[_]]
    // We are manually maintaining a stack here to prevent StackOverflowError
    // caused by recursively visiting
//    3.防止递归调用的时候出现StackOverflowError异常
//    自己分配堆
...
//    获取当前Job的最大分区数，即maxPartitions，根据maxPartitions，
//    确认我们没有在一个不存在的partitions上执行任务
...
//      创建jobwaiter,在Job运行被阻塞，直到job完成或者被取消
